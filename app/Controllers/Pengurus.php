...
class Pengurus extends BaseController
{
	...
	public function getAll()
	{
		...
		$result = $this->pengurusModel->select()->findAll();
...
$value->id_anggota,
	...
	public function getOne()
	{
		...
		if ($this->validation->check($id, 'required|numeric')) {
			...
			$data = $this->pengurusModel->where('id' ,$id)->first();
...
$fields['kode_bagian'] = $this->request->getPost('kode_bagian');
$fields['id_anggota'] = $this->request->getPost('id_anggota');


        $this->validation->setRules([
			            'kode_bagian' => ['label' => 'Kode bagian', 'rules' => 'required|min_length[0]|max_length[10]'],
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->pengurusModel->insert($fields)) {
...
$fields['id_anggota'] = $this->request->getPost('id_anggota');
        ...
        if ($this->validation->run($fields) == FALSE) {
            ...
            if ($this->pengurusModel->update($fields['id'], $fields)) {
	...
	public function remove()
	{
		...
		if (!$this->validation->check($id, 'required|numeric')) {
			...
			if ($this->pengurusModel->where('id', $id)->delete()) {
